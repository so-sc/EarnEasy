...
const LandingPage = () => {
  ...
  return (
    <Box sx={{ bgcolor: "#fff", fontFamily: "'Poppins',sans-serif" }}>
        {/* Header */}
        <Box sx={{background:"linear-gradient(to bottom,#7affff,#ffffff)",
          textAlign:"center", py:6,}}>
           <Container maxWidth="md">
           <Typography variant="h2" fontWeight="bold" textAlign="center" mb={4} sx={{fontFamily:"'Playfair Display',serif"}}>Earn Easy</Typography>
           <Box
            component="img"
            src="/images/hero.png"
            alt="Hero"
            mt={4}
            sx={{ width:200,borderRadius:"50%", boxShadow:3,background:"#2ff2fd" }}
           />
        ...
        <Typography variant="h5" fontWeight="bold" textAlign="center" mb={4} sx={{fontFamily:"'Playfair Display',serif"}}>
          How It Works...
        </Typography>
        <Container maxWidth="md">
        <Grid container spacing={3} justifyContent="center">
          {["Create an Account", "List or Find Items", "Connect & Earn"].map(
            (step, i) => (
              <Grid key={i} gridSize={{ xs: 12, sm: 4 }}>
                <Box
                  sx={{
                    bgcolor: "#e3f2fd",
                    p: 2,
        ...
        <Grid container spacing={2} mb={5}>
          {categories.map((item, i) => (
            <Grid key={i} gridSize={{ xs: 12, sm: 4 }}>
              ...
              image={item.img}
              alt={item.name}
              sx={{ height: 150, width: '100%', objectFit: 'cover' }} // Adjust height and width here
                />
